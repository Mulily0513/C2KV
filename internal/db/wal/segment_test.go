...
func TestSegmentFile_newSegmentFile(t *testing.T) {
	...
	assert.EqualValues(t, segment.segmentOffset, 0)
...
func TestSegmentFile_BlockWrite(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.EqualValues(t, bytesCount, segment.segmentOffset)
			assert.EqualValues(t, bytesCount, segment.blocksOffset)
			assert.GreaterOrEqual(t, segment.blockNums, 1)

			got := readEntriesBySeg(segment)
			assert.EqualValues(t, tt.ents, got)
		})
	}
}
...
func TestSegmentFile_SerialWrite(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.EqualValues(t, totalCounts, segment.segmentOffset)
