...
type KV struct {
	...
	KeySize  uint32
	Key      []byte
	// Data is decoded view used by read paths and legacy apply path.
	Data *Data
	// DataBytes is an optional pre-encoded Data payload (Index+Timestamp+Type+Value).
	// When set, storage apply path can write it directly to memtable without re-encoding.
	DataBytes []byte
}
...
func EncodeKV(kv *KV) []byte {
	...
	payloadLen := IndexSize + TimeStampSize + TypeSize + valueLen
	buf := make([]byte, ApplySigSize+KeySize+keyLen+payloadLen)
	copy(buf[:ApplySigSize], kv.ApplySig)
	binary.LittleEndian.PutUint32(buf[ApplySigSize:ApplySigSize+KeySize], uint32(keyLen))
	copy(buf[ApplySigSize+KeySize:], kv.Key)

	payload := buf[ApplySigSize+KeySize+keyLen:]
	binary.LittleEndian.PutUint64(payload[:IndexSize], data.Index)
	binary.LittleEndian.PutUint64(payload[IndexSize:IndexSize+TimeStampSize], uint64(data.TimeStamp))
	payload[IndexSize+TimeStampSize] = byte(data.Type)
	copy(payload[IndexSize+TimeStampSize+TypeSize:], data.Value)
	return buf
}

// EncodeProposalKV serializes one client proposal directly without allocating
// intermediate KV/Data objects. Wire format is kept identical to EncodeKV.
func EncodeProposalKV(proposeID uint64, key, value []byte, isDelete bool, ts int64) []byte {
	keyLen := len(key)
	valueLen := len(value)
	payloadLen := IndexSize + TimeStampSize + TypeSize + valueLen
	buf := make([]byte, ApplySigSize+KeySize+keyLen+payloadLen)

	// ApplySig keeps existing layout (36B reserved), with first 8 bytes storing id.
	binary.LittleEndian.PutUint64(buf[:8], proposeID)
	binary.LittleEndian.PutUint32(buf[ApplySigSize:ApplySigSize+KeySize], uint32(keyLen))
	copy(buf[ApplySigSize+KeySize:], key)

	payload := buf[ApplySigSize+KeySize+keyLen:]
	// Raft index is patched at apply time.
	binary.LittleEndian.PutUint64(payload[:IndexSize], 0)
	binary.LittleEndian.PutUint64(payload[IndexSize:IndexSize+TimeStampSize], uint64(ts))
	if isDelete {
		payload[IndexSize+TimeStampSize] = byte(TypeDelete)
	} else {
		payload[IndexSize+TimeStampSize] = byte(TypeInsert)
	}
	copy(payload[IndexSize+TimeStampSize+TypeSize:], value)
	return buf
}
