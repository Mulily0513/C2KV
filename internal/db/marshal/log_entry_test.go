...
func TestEncodeProposalKV(t *testing.T) {
	id := uint64(12345)
	key := []byte("k1")
	value := []byte("v1")
	ts := int64(1700000000)

	encoded := EncodeProposalKV(id, key, value, false, ts)
	decoded := DecodeKV(encoded)

	if got := binary.LittleEndian.Uint64(decoded.ApplySig[:8]); got != id {
		t.Fatalf("proposal id mismatch, got=%d want=%d", got, id)
	}
	assert.EqualValues(t, key, decoded.Key)
	assert.EqualValues(t, uint64(0), decoded.Data.Index)
	assert.EqualValues(t, ts, decoded.Data.TimeStamp)
	assert.EqualValues(t, int8(TypeInsert), decoded.Data.Type)
	assert.EqualValues(t, value, decoded.Data.Value)
}
