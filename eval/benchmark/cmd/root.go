...
var RootCmd = &cobra.Command{
	...
	Long:  `benchmark is a low-level benchmark tools for c2kv.It uses gRPC client directly and does not depend on c2kv client library.`,
}
...
func init() {
	...
	RootCmd.PersistentFlags().StringVar(&outputJSONPath, "output-json", "", "write benchmark result summary to JSON file")
	RootCmd.PersistentFlags().StringVar(&outputCSVPath, "output-csv", "", "write per-second time series to CSV file")
	RootCmd.PersistentFlags().StringVar(&clusterName, "cluster-name", "", "cluster label in benchmark outputs")
	RootCmd.PersistentFlags().StringVar(&engineTag, "engine-tag", "c2kv", "engine label in benchmark outputs")
}
